...
(emms-devel)            ;选择开发者模式
;; 目录
(make-directory "~/.emacs.d/config-file/emms/" t)
(setq emms-directory "~/.emacs.d/config-file/emms/")           ;设置EMMS的目录
(setq emms-history-file "~/.emacs.d/config-file/emms/history") ;播放列表历史记录
(setq emms-cache-file "~/.emacs.d/config-file/emms/cache")     ;缓存文件
(setq emms-stream-bookmarks-file "~/.emacs.d/config-file/emms/streams") ;网络电台保存文件
(setq emms-score-file "~/.emacs.d/config-file/emms/scores")             ;分数文件
...
(emms-mode-line-enable)                ;不在Mode-line上显示歌曲信息
...
;;  '("s" "m" "u" "M-<" "M->") emms-playlist-mode-map)                ;卸载按键
;; (lazy-load-set-keys
;;  '(
