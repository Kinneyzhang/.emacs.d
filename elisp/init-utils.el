        ...
        (pcase ofield
          ((pred vb-find-ifield)
           (let* ((ifield (vb-find-ifield ofield))
             ...
             (setq res (concat res (vb-trans-info ifield ofield iNo oNo) "\n"))))
          ((and (let iNo (vb-get-iNo ofield))
                (guard iNo))
           (setq res (concat res (vb-trans-info ofield ofield iNo oNo) "\n")))
          (_ (setq res (concat res (vb-trans-info "null" ofield " " oNo) "\n"))))))
    res))
...
(defun vb-fetch-finalize ()
  ...
  (erase-buffer)
  (insert (vb-trans-field vb-ifileds vb-ofileds)))

(defun vb-fetch-cancel ()
  (interactive)
  (kill-buffer vb-fetch-buffer))

(defun vb-fetch-all-vars ()
  (interactive)
